...
class InferClassTag extends FunSpec {
  ...
  it("") {
    ...
    assert(ctg1.toString == "com.tribbloids.spike.spike.InferImplicit.InferClassTag$Sub1")

    val ctg2 = infer(Some(new Sub11), Some(new Sub22), None)
    assert(ctg2.toString == "com.tribbloids.spike.spike.InferImplicit.InferClassTag$Super")
  }
}
